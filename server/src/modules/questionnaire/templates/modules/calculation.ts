...
import { currentTerm, FV, currentDate, chartFill } from "../../questionnaire.interfaces";
...
const currentPeriod: currentDate = (id) => {
  return periods.find(p => p.id === id).period;
};

const currentMonths: currentDate = (id) => {
...
const FV1: FV = (resourses, profitability, term) => {
...
const dateFilterChart = (date: Date, period: number) => {
...
const chartFill: chartFill = (resourses, term, period, month, profitability, targetCurrency, targetCurrencySign, getCurrectCurrency, course, currentR) => {
  if (period === 0) return [];
  let chart: Array<dynamicsObject> = [];
  let capital = 0;
  const date = new Date();
