...
function getCorrectCurrency(instruments, currency, course) {
...
;
function getOtherCurrency(amount, currency, baseCurrency) {
    const rates = (0, common_1.getCurrentExchangeRates)();
    return (amount / rates[currency] * rates[baseCurrency]).toFixed(0);
}
function getUSDCurrency(amount, currency) {
    const rates = (0, common_1.getCurrentExchangeRates)();
    return (amount / rates[currency]).toFixed(0);
}
function getRUBCurrency(amount, currency) {
    const rates = (0, common_1.getCurrentExchangeRates)();
    return (amount / rates[currency] * rates['RUB']).toFixed(0);
}
function getCurrentInstruments(instruments, type, filter, filterElements, course) {
    if (filter && filterElements) {
        if (type.includes('section_') || type.includes('instrument_type_'))
            return instruments.filter(i => i[`class_${course}_id`] === 'stock' && i['section_' + course] && filterElements.includes(i[filter]));
    }
    else {
        if (type === 'matdate')
            return instruments.filter((i) => i[`class_${course}_id`] === "bond");
        if (type.includes('section_'))
            return instruments.filter(i => i[`class_${course}_id`] === 'stock' && i['section_' + course]);
        if (type.includes('instrument_type_'))
            return instruments.filter(i => i[`class_${course}_id`] === 'bond' && i['instrument_type_' + course]);
    }
    return instruments;
}
function getCorrectStructurePercents(pid, need, course, assets, instruments, type, filter, filterElements) {
    const result = {};
    ...
    copy.forEach(instrument => {
        ...
        if (!result[correct_type] && type !== 'matdate')
            result[correct_type] = 0;
        if (pid === 'expert' && need) {
            ...
            const currentAmount = course === enums_1.COURSES.ONE ?
            ...
            if ((type.includes('class_') || type.includes('section_')) && instrument[`instrument_type_${course}`] === 'Фонды смешанных активов') {
                ...
                if (mixedAsset) {
                    ...
                    if (type.includes('class_')) {
                        classes.forEach(classAsset => {
                            if (!result[classAsset[course]])
                                result[classAsset[course]] = 0;
                ...
                if (+date) {
                    ...
                    if (correctDate <= date) {
                        if (!result['Долгосрочные'])
                            result['Долгосрочные'] = 0;
                        result['Долгосрочные'] += currentAmount;
                    }
                    else {
                        if (!result['Краткосрочные'])
                            result['Краткосрочные'] = 0;
    ...
    if (result && result['Акции'] && type === filter && type === `instrument_type_${course}`) {
        Object.defineProperty(result, 'Акции отдельных компаний', Object.getOwnPropertyDescriptor(result, 'Акции'));
        delete result['Акции'];
    }
    return result;
}
function sortInstrumentsSection(instruments) {
    return instruments.sort((a, b) => {
        if (a.section_two > b.section_two)
            return 1;
        if (a.section_two < b.section_two)
            return -1;
        return 0;
...
function sortInstruments(type, instruments) {
    if (type === 'stock') {
        return [
    ...
    if (type === 'bond') {
        ...
        for (const sort of sorting) {
            for (const instrument of instruments) {
                if (instrument.instrument_type_two === sort)
                    array = [...array, instrument];
            }
        }
        for (const instrument of instruments) {
            if (!array.find(e => e.name === instrument.name))
                array = [...array, instrument];
        }
        return array;
    }
...
function getProtfolioDescription(key, course) {
    return [
...
exports.default = (target, course, assets, FV) => {
    (0, common_1.getConvert)();
    let portfolios = [];
    let index = 0;
...
function getCorrectComment(portfolio, name) {
    const section = portfolio.sections.find(s => s.default === name);
    const array = (section && section.modules[0]) ? section.modules[0].data.comment.trim().split('\n') : [];
    const text = array.reduce((acc, element) => {
