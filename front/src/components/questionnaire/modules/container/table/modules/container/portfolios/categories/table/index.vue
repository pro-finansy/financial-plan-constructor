  ...
  <div class="table" :class="{green: portfolioId === 'existingPortfolio', yellow: portfolioId === 'studentPortfolio'}">
    <div class="table--container" v-if="core">
      <common-table
        ...
        :instruments="getCurrentInstruments(_type.id)"
        :type="_type"
      ></common-table>
    </div>
    <div class="table--container" v-else>
      <common-table
        ...
        :portfolioId="portfolioId"
        :length="modules.length"
        :instruments="modules.filter((m) => m.data.name).map((m) => m.data)"
      ></common-table>
    </div>
  </div>
</template>
...
import { TABLE_BOND_SORT, TABLE_CORE_TYPES, TABLE_CORE_TYPE_MIXED } from "@/components/questionnaire/modules/container/modules/target/portfolio/constants";
